...
def connect_to_dotproduct_local(
    ...
    """Connect to the local Weaviate instance used by the dotproduct stack.

    Defaults are environment-aware so code can run both inside the Docker network and on the host.
    """
    resolved_host = host or os.getenv("WEAVIATE_HOST", "localhost")
    resolved_port = int(port or os.getenv("WEAVIATE_PORT", "8080"))
    resolved_grpc_port = int(grpc_port or os.getenv("WEAVIATE_GRPC_PORT", "50051"))
